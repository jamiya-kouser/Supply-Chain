"""
Driver service layer — business logic for driver operations.

All database access goes through Motor async queries.
"""
...
class DriverService:
    ...
    async def get_driver_by_id(self, driver_id: str) -> Optional[Dict[str, Any]]:
        """Fetch a single driver by their driver_id."""
        collection = get_drivers_collection()
        return await collection.find_one({"driver_id": driver_id}, {"_id": 0})

    async def get_driver_shipments(self, driver_id: str) -> Dict[str, Any]:
        """Return the shipment assignments for a driver."""
        shipments_collection = get_shipments_collection()
        shipments = await shipments_collection.find(
            {
                "carrier.driver_id": driver_id,
